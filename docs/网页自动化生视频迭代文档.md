...
class LovartAutomation:
    ...
    def __init__(self, email_receiver, account_pool):
        self.email_receiver = email_receiver
        self.account_pool = account_pool
    
    def register_account(self, account, headless=False) -> bool:
        """自动注册（选择器需实测确认）"""
        browser = FingerprintBrowser(account.id)
        ctx, page = browser.launch(headless)
        try:
            page.goto("https://www.lovart.ai")
            # 1. 点击登录/注册
            # 2. 填写邮箱
            # 3. 点击发送验证码
            # 4. IMAP 获取验证码
    ...
    def generate_video(self, account, image_path: str, prompt: str, headless=False):
        """生成视频（选择器需实测确认）"""
        browser = FingerprintBrowser(account.id)
        ctx, page = browser.launch(headless)
        try:
            page.goto(self.CANVAS_URL)
            # 1. 上传图片
            # 2. 填写 Prompt（带前缀）
            # 3. 点击生成
            # 4. 等待完成并下载
