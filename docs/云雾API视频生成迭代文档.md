...
| 文件 | 改动类型 | 说明 |
|------|----------|------|
| `Step3_DeconstructionReview.tsx` | 修改 | 「供应商」改名为「生图供应商」，新增「生视频配置」按钮 |
| `VideoConfigModal.tsx` | 新建 | 生视频配置弹窗组件 |
| `Step7_VideoGen.tsx` | 修改 | 添加状态栏、镜头生成按钮 |
| `lib/stores/videoGenStore.ts` | 新建 | 视频生成状态管理 |
| `lib/services/yunwuVideo.ts` | 新建 | 云雾 API 调用封装 |
| `hooks/useVideoProgress.ts` | 新建 | SSE 进度订阅 Hook |
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| **生成模式** | Toggle | Lovart | Lovart 自动化 / 云雾 API |
...
### Phase 1: 后端增强 (1天) ✅
- [x] 添加配置管理 API（保存/读取 API Key 和默认参数）
- [x] 实现并发任务执行器
- [x] 添加 SSE 进度推送端点
- [x] 添加连接测试 API

### Phase 2: 前端基础 (1天) ✅
- [x] 创建 `videoGenStore` 状态管理
- [x] 实现模式切换 UI
- [x] 实现 API 配置弹窗
- [x] 集成配置保存
...
### Phase 4: 进度与并发 (1天) ✅
- [x] 实现 SSE 进度订阅（云雾模式使用 SSE，Lovart 模式使用轮询）
- [x] 进度条动画（带百分比显示）
- [x] 并发执行控制
- [ ] 执行日志面板（可选）
