...
### 🛡️ 前3秒特殊约束：3秒常识测试 (3-Second Common Sense Test)

**前3秒的特殊地位**：
- 可以使用**"突发事件"开场**，无需铺垫（例如：突然喷液体、突然看到异象）
- 但**必须通过3秒常识测试**，防止逻辑崩塌
...
| 口吐蓝液 | ✅ | ✅低（喝多了） | ❌ | ✅可用 |
| 海面巨影 | ✅ | ✅低（有怪物） | ❌ | ✅可用 |
| 杯子无故裂 | ⚠️勉强 | ❌中（需思考原因） | ✅需要 | ❌禁用 |
| 杯中倒远物 | ❌ | ❌高（违反光学） | ✅需要 | ❌禁用 |
| 眼睛变红发光 | ⚠️科幻可接受 | ✅低（生气/异能） | ❌ | ✅可用 |

**替代方案生成机制**：
如果Hook元素未通过测试，**必须**生成符合常识的替代元素。

**禁止模式**：
- ❌ "先射箭再画靶"：先决定加元素，用户质疑后才编造5条物理解释
