...
|------|----------|----------|------|------------|---------------------------|---------------------------|
| 1 | 0.000s | 1.234s | 1.234s | frame_001.jpg | [景别]中景 [视角]平视<br>[主体与形象]【粉色短发男生】<br>[表情]【粉色短发男生】：温和微笑<br>[动作]【粉色短发男生】位于画面中央，站立，双手自然垂放身体两侧，头部微微右倾，眼神直视镜头<br>[环境]纯白背景，左前方45度柔光在面部右侧形成柔和阴影<br>[时间]白天 [天气]晴 | 固定镜头，【粉色短发男生】突然睁大眼睛, 嘴巴张成O型, 头部快速后仰, 右手抬起指向画面左侧, 身体向右转动90度，表情由温和微笑变为震惊难以置信。 |
| 2 | 1.234s | 3.567s | 2.333s | frame_002.jpg | [景别]中景 [视角]过肩视角<br>[主体与形象]【粉色短发男生】；【蓝色长发女生】<br>[表情]【粉色短发男生】：无表情（背影）；【蓝色长发女生】：惊喜期待<br>[动作]【粉色短发男生】位于画面左侧（清晰），背对镜头站立；【蓝色长发女生】位于画面右侧（清晰），正面站立，双手捧着红色礼物盒，右手搭在【粉色短发男生】左肩上<br>[环境]室内客厅，午后暖阳从画面右侧斜照，背景木质书架虚化<br>[时间]白天 [天气]晴 | 固定镜头，【蓝色长发女生】低头看礼物盒, 缓缓打开盒盖, 随即抬头看向【粉色短发男生】, 张开双臂拥抱，表情由惊喜期待变为感动落泪。 |
| 3 | 3.567s | 5.800s | 2.233s | frame_003.jpg | [景别]特写 [视角]俯视30度<br>[主体与形象]【蓝色长发女生】<br>[表情]【蓝色长发女生】：专注认真，眉头微皱<br>[动作]【蓝色长发女生】位于画面中央（清晰），双手高举不锈钢菜刀过头顶，低头盯着面前木质砧板上的红色西瓜<br>[环境]厨房，顶部暖黄灯光照亮砧板区域，背景浅木色橱柜虚化<br>[时间]白天 [天气]晴 | 固定镜头，【蓝色长发女生】用力向下劈砍, 西瓜裂开成两半, 汁水四溅, 随即抬头看向镜头, 放下菜刀，表情由专注认真变为得意满足。 |

##### 字段定义与格式规范

**画面提示词 (Image Prompt)**:
- **用途**: 为文生图AI设计，生成【起始静止帧】
- **标签结构**（按顺序排列，每个标签用 \`<br>\` 换行）：
...
- **角色说明**: 包含年龄、外貌、服装、身高等可见特征，**根据实际画面详略得当**
- **画面提示词 (Image Prompt)**: 
  - **标签结构必须完整**：[景别][视角] → [主体与形象] → [表情] → [动作] → [环境] → [时间][天气]
  - **多角色场景**：每个角色须标注位置和虚实状态；物理接触在执行者姿态中引用被接触者
- **视频提示词 (Video Prompt)**: 
  - **必须包含元素**：运镜方式、主体特征代词、动作序列（逗号分隔）、表情变化
  - **格式要求**：严格遵循 \`**运镜方式**，【主体】[动作1], [动作2], [动作3]，表情由[起始]变为[结束]\` 格式
...
## 🔍 拆解格式自检清单 (Format Check)

拆解视频后，检查**输出格式**是否正确。

> 💡 **核心原则**：拆解任务是**如实记录**原视频内容，不评判原视频是否合理。

### 一、首帧与视频分离检查

#### 1.1 状态匹配
- [ ] initial_frame = **静态起始状态**（该镜头第一帧的静止画面）
- [ ] visual_changes = **动态变化过程**（该镜头内发生的动作/变化）
- [ ] 首帧状态 + 视频变化 = **逻辑闭环**

#### 1.2 常见格式错误
- [ ] ❌ 首帧描述了"结果"而非"起点"（应描述镜头切入瞬间）
- [ ] ❌ 视频描述了"状态"而非"变化"（应描述动态过程）
- [ ] ❌ 表情/动作在首帧已完成，视频却说"开始..."

### 二、标签结构检查

- [ ] initial_frame 是否包含完整标签：[景别][视角] → [主体与形象] → [表情] → [动作] → [环境] → [时间][天气]
- [ ] 标签顺序是否正确
- [ ] 角色是否使用【特征+性别】格式命名
...
| 问题位置 | 问题类型 | 问题描述 | 建议修复 |
|----------|----------|----------|----------|
| #3 首帧 | 首帧/视频混淆 | 首帧描述了动作过程"正在跑" | 改为静态起点"双腿迈开呈跑步姿态" |
