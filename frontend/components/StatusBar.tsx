...
export default function StatusBar() {
    ...
    return (
        <div className="flex items-center justify-between flex-wrap gap-4 px-1 py-3 text-[var(--color-text-primary)]">
            {/* Video Info */}
            <div className="flex items-center gap-4 text-sm">
                <div className="flex items-center gap-1.5">
                    <Film size={16} className="text-[var(--color-blue-500)]" />
            ...
            <div className="flex items-center gap-3 text-sm">
                <div className="flex items-center gap-1.5">
                    <Scissors size={16} className="text-[var(--color-success)]" />
                    <span>{visibleCuts} 个镜头</span>
                </div>
                {hiddenCount > 0 && (
                    <span className="text-xs text-[var(--color-text-secondary)]">已隐藏 {hiddenCount} 段</span>
                )}
            </div>
        </div>
    );
